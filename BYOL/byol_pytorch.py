...
class RandomGridDistortion(nn.Module):
    def __init__(self, grid_size=20, max_distortion=5):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for i in range(grid_h):
            for j in range(grid_w):
                y_start, y_end = i * self.grid_size, min((i + 1) * self.grid_size, h)
                x_start, x_end = j * self.grid_size, min((j + 1) * self.grid_size, w)

                y_distortion = torch.randint(
                    -self.max_distortion, self.max_distortion + 1, (1,), device=x.device
                ).item()
                x_distortion = torch.randint(
                    -self.max_distortion, self.max_distortion + 1, (1,), device=x.device
                ).item()

                y_distortion = max(min(y_distortion, self.grid_size), -self.grid_size)
                x_distortion = max(min(x_distortion, self.grid_size), -self.grid_size)

                distorted_x[:, :, y_start:y_end, x_start:x_end] = torch.roll(
                    distorted_x[:, :, y_start:y_end, x_start:x_end],
                    shifts=(y_distortion, x_distortion),
                    dims=(2, 3),
                )
...
class BYOL(nn.Module):
    def __init__(
        ...
        DEFAULT_AUG = torch.nn.Sequential(
            ...
            RandomApply(
                T.GaussianBlur((3, 3), (1.0, 2.0)), p = 0.2),
            RandomGridDistortion(grid_size=20, max_distortion=5),
            T.Normalize(
